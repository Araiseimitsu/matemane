  ...
  if (scheduleBtn) {
    scheduleBtn.addEventListener("click", async function () {
      ...
      scheduleBtn.innerHTML =
        '<i class="fas fa-spinner fa-spin mr-2"></i> 実行中...';
...
let ordersAllItems = []; // 全データを保持

function initializeOrdersTab() {
  document
    .getElementById("refreshOrdersBtn")
    ?.addEventListener("click", () => loadOrders(1));
  document
    .getElementById("resetOrderFiltersBtn")
    ?.addEventListener("click", resetOrderFilters);

  // リアルタイム検索（入力時にdebounceで検索実行）
  document.getElementById("orderNumberFilter")?.addEventListener(
    "input",
    debounce(() => renderFilteredOrders(1), 300),
  );
  document.getElementById("orderMaterialFilter")?.addEventListener(
    "input",
    debounce(() => renderFilteredOrders(1), 300),
  );
  document
    .getElementById("orderStatusFilter")
    ?.addEventListener("change", () => renderFilteredOrders(1));
  document.getElementById("orderSupplierFilter")?.addEventListener(
    "input",
    debounce(() => renderFilteredOrders(1), 300),
  );
}
...
async function loadOrders(page = 1) {
  ...
  try {
    // 全データを取得（ページサイズを大きくして一括取得）
    const response = await fetch(
      `/api/purchase-orders/?page=1&page_size=10000`,
    );
    ...
    // 全データを保存
    ordersAllItems = data.items;

    // フィルター適用して表示
    renderFilteredOrders(page);
  } catch (error) {
    loading.classList.add("hidden");
...
function renderFilteredOrders(page = 1) {
  const tableBody = document.getElementById("ordersTableBody");
  const empty = document.getElementById("ordersEmpty");

  // フィルター条件取得
  const orderNumber =
    document.getElementById("orderNumberFilter")?.value.toLowerCase() || "";
  const material =
    document.getElementById("orderMaterialFilter")?.value.toLowerCase() || "";
  const status = document.getElementById("orderStatusFilter")?.value;
  const supplier =
    document.getElementById("orderSupplierFilter")?.value.toLowerCase() || "";

  // フィルタリング
  const filteredOrders = ordersAllItems.filter((order) => {
    const matchOrderNumber =
      !orderNumber || order.order_number.toLowerCase().includes(orderNumber);
    const matchSupplier =
      !supplier || order.supplier.toLowerCase().includes(supplier);
    ...
    const firstItem =
      Array.isArray(order.items) && order.items.length > 0
        ? order.items[0]
        : null;
    const materialSpec = firstItem ? firstItem.item_name.toLowerCase() : "";
    const matchMaterial = !material || materialSpec.includes(material);

    return matchOrderNumber && matchSupplier && matchStatus && matchMaterial;
  });
  ...
  if (filteredOrders.length === 0) {
    tableBody.innerHTML = "";
    empty.classList.remove("hidden");
    renderPagination("orders", 1, 1, renderFilteredOrders);
    return;
  }
  ...
  currentOrdersPage = Math.min(page, totalPages);
...
function initializeReceivingTab() {
  document
    .getElementById("refreshReceivingBtn")
    ?.addEventListener("click", () => loadReceivingItems(1));
  document
    .getElementById("resetReceivingFiltersBtn")
    ?.addEventListener("click", resetReceivingFilters);
  document
    .getElementById("includeInspectedFilter")
    ?.addEventListener("change", () => loadReceivingItems(1));

  // リアルタイム検索（入力時にdebounceで検索実行）
  document.getElementById("receivingOrderNumberFilter")?.addEventListener(
    "input",
    debounce(() => renderFilteredReceivingItems(1), 300),
  );
  document.getElementById("receivingSupplierFilter")?.addEventListener(
    "input",
    debounce(() => renderFilteredReceivingItems(1), 300),
  );
  document.getElementById("receivingMaterialFilter")?.addEventListener(
    "input",
    debounce(() => renderFilteredReceivingItems(1), 300),
  );
  ...
  // 手入力で入庫ボタン
  document
    .getElementById("manualReceiveBtn")
    ?.addEventListener("click", showManualReceiveModal);

  // 手入力用入庫確認モーダル
  document
    .getElementById("closeManualReceiveModal")
    ?.addEventListener("click", hideManualReceiveModal);
  document
    .getElementById("cancelManualReceive")
    ?.addEventListener("click", hideManualReceiveModal);
  document
    .getElementById("manualReceiveForm")
    ?.addEventListener("submit", handleManualReceive);

  // 入庫確認モーダル
  document
    .getElementById("closeReceiveModal")
    ?.addEventListener("click", hideReceiveModal);
  document
    .getElementById("cancelReceive")
    ?.addEventListener("click", hideReceiveModal);
  document
    .getElementById("receiveForm")
    ?.addEventListener("submit", handleReceive);
...
async function loadReceivingItems(page = 1) {
  ...
  try {
    const includeInspected = document.getElementById("includeInspectedFilter")
      ?.checked
      ? "true"
      : "false";
    const response = await fetch(
      `/api/purchase-orders/pending-or-inspection/items/?include_inspected=${includeInspected}`,
    );
    ...
    // フィルター適用して表示
    renderFilteredReceivingItems(page);
  } catch (error) {
    loading.classList.add("hidden");
...
function createReceivingItemRow(item) {
  ...
  const orderNumber = item.purchase_order
    ? item.purchase_order.order_number
    : "-";
  const supplier = item.purchase_order ? item.purchase_order.supplier : "-";
  const orderDate =
    item.purchase_order && item.purchase_order.order_date
      ? new Date(item.purchase_order.order_date).toLocaleDateString("ja-JP")
      : "-";
  const expectedDate =
    item.purchase_order && item.purchase_order.expected_delivery_date
      ? new Date(item.purchase_order.expected_delivery_date).toLocaleDateString(
          "ja-JP",
        )
      : "-";
...
function renderFilteredReceivingItems(page = 1) {
  const tableBody = document.getElementById("receivingItemsTableBody");
  const empty = document.getElementById("receivingEmpty");

  // フィルター条件取得
  const supplier =
    document.getElementById("receivingSupplierFilter")?.value?.toLowerCase() ||
    "";
  const orderNumber =
    document
      .getElementById("receivingOrderNumberFilter")
      ?.value?.toLowerCase() || "";
  const material =
    document.getElementById("receivingMaterialFilter")?.value?.toLowerCase() ||
    "";

  // フィルタリング
  const filteredItems = receivingAllItems.filter((item) => {
    const itemSupplier = item.purchase_order
      ? item.purchase_order.supplier.toLowerCase()
      : "";
    const itemOrderNumber = item.purchase_order
      ? item.purchase_order.order_number.toLowerCase()
      : "";
    const itemMaterial = item.item_name.toLowerCase();

    const matchSupplier = !supplier || itemSupplier.includes(supplier);
    const matchOrderNumber =
      !orderNumber || itemOrderNumber.includes(orderNumber);
    const matchMaterial = !material || itemMaterial.includes(material);

    return matchSupplier && matchOrderNumber && matchMaterial;
  });
  ...
  if (filteredItems.length === 0) {
    tableBody.innerHTML = "";
    empty.classList.remove("hidden");
    renderPagination("receiving", 1, 1, renderFilteredReceivingItems);
    return;
  }
  ...
  // ページネーション計算
  const totalPages = Math.ceil(filteredItems.length / receivingPageSize);
  ...
  renderPagination(
    "receiving",
    currentReceivingPage,
    totalPages,
    renderFilteredReceivingItems,
  );
}
...
function resetReceivingFilters() {
  ...
  renderFilteredReceivingItems(1);
}
...
async function showReceiveModal(itemId) {
  try {
    const response = await fetch(
      `/api/purchase-orders/pending-or-inspection/items/`,
    );
    const items = await response.json();
    ...
    const orderResponse = await fetch(
      `/api/purchase-orders/${item.purchase_order_id}`,
    );
    const order = await orderResponse.json();
    ...
    try {
      ...
      if (isReceived) {
        const prevRes = await fetch(
          `/api/purchase-orders/items/${itemId}/receive/previous/`,
        );
        if (prevRes.ok) {
          const prev = await prevRes.json();

          // 入力要素
          const diameterCalcInput =
            document.getElementById("diameterCalcInput");
          ...
          const receivedDateField = document.querySelector(
            'input[name="received_date"]',
          );

          // 計算用パラメータ
          if (diameterCalcInput && typeof prev.diameter_mm === "number")
            diameterCalcInput.value = prev.diameter_mm;
          if (shapeCalcSelect && prev.shape) shapeCalcSelect.value = prev.shape;
          if (densityField && typeof prev.density === "number")
            densityField.value = prev.density;
          ...
          if (lengthField && typeof prev.length_mm === "number")
            lengthField.value = prev.length_mm;
          ...
          // 購入月
          const purchaseMonthField = document.querySelector(
            'input[name="purchase_month"]',
          );
          ...
          if (receiveForm) {
            let isEditInput = receiveForm.querySelector(
              'input[name="is_edit"]',
            );
            if (!isEditInput) {
            ...
            let prevLotsInput = receiveForm.querySelector(
              'input[name="previous_lot_numbers"]',
            );
            if (!prevLotsInput) {
    ...
    currentOrderedQuantity =
      item.order_type === "weight" ? 0 : item.ordered_quantity;
    document.getElementById("orderedQuantity").textContent =
      currentOrderedQuantity > 0 ? currentOrderedQuantity : "-";
...
function updatePurchaseMonth() {
  ...
  if (!receivedDateInput || !purchaseMonthInput || !receivedDateInput.value)
    return;
...
function setupConversionListeners() {
  ...
  if (quantityInput) {
    quantityInput.oninput = function () {
      if (this.value) {
        weightInput.value = "";
        const quantityRadio = document.querySelector(
          'input[name="input_method"][value="quantity"]',
        );
    ...
    weightInput.oninput = function () {
      if (this.value) {
        ...
        const weightRadio = document.querySelector(
          'input[name="input_method"][value="weight"]',
        );
...
async function loadDensityPresets() {
  try {
    const response = await fetch(
      "/api/density-presets/?is_active=true&limit=100",
    );
    if (!response.ok) {
    ...
    selectElement.onchange = function () {
      ...
      if (this.value && densityInput) {
        ...
        if (
          ...
          showToast(
            `比重プリセット「${selectedOption.dataset.name}」を適用しました`,
            "success",
          );
        }
...
async function handleReceive(event) {
  ...
  try {
    ...
    for (let i = 1; i <= lotRowCounter; i++) {
      ...
      const lotNumberInput = lotRow.querySelector(
        `input[name="lot_number_${i}"]`,
      );
      const quantityInput = lotRow.querySelector(
        `input[name="lot_quantity_${i}"]`,
      );
      const weightInput = lotRow.querySelector(`input[name="lot_weight_${i}"]`);
      const locationElement =
        lotRow.querySelector(`select[name="lot_location_${i}"]`) ||
        lotRow.querySelector(`input[name="lot_location_${i}"]`);
      ...
      // 空文字列をnullに変換
      const quantity =
        quantityStr && quantityStr.trim() !== "" ? quantityStr : null;
      ...
      const lotData = {
        ...
        location_id:
          location && location.trim() !== "" ? parseInt(location) : null,
    ...
    try {
      ...
      for (const lot of lots) {
        ...
        const isUpdatingExisting =
          isEdit && previousLotNumbers.includes(lot.lot_number);
        const methodToUse = isUpdatingExisting ? "PUT" : "POST";

        console.log(
          `ロット ${lot.lot_number}: ${methodToUse} (isEdit=${isEdit}, 既存=${isUpdatingExisting})`,
        ); // デバッグ
      ...
      for (const deletedLotNumber of removedLotNumbers) {
        try {
          const deleteRes = await fetch(
            `/api/purchase-orders/items/${itemId}/receive/${encodeURIComponent(deletedLotNumber)}/`,
            {
              method: "DELETE",
            },
          );
          ...
          if (!deleteRes.ok) {
            ...
            try {
              ...
              deleteMsg =
                deleteMsg?.substring(0, 200) ||
                `ステータス ${deleteRes.status}`;
            }
          ...
          deletionErrors.push(
            `ロット ${deletedLotNumber}: ${deleteError.message || deleteError}`,
          );
        }
      }
...
async function showManualReceiveModal() {
  try {
    ...
    // 入荷日のデフォルトを本日に設定
    const receivedDateField = document.getElementById(
      "manualReceivedDateInput",
    );
    ...
    // 計算用パラメータ変更時に合計を更新
    const diameterCalcInput = document.getElementById(
      "manualDiameterCalcInput",
    );
...
function hideManualReceiveModal() {
  ...
  // 送信ボタンの状態をリセット
  const submitButton = document.querySelector(
    '#manualReceiveForm button[type="submit"]',
  );
  if (submitButton) {
    submitButton.disabled = false;
    submitButton.innerHTML = "入庫確定";
...
function setupManualPurchaseMonthAutoUpdate() {
  ...
  const purchaseMonthInput = document.getElementById(
    "manualPurchaseMonthInput",
  );
...
function addManualLotRow(
  ...
  const quantityInput = row.querySelector(
    `input[name="manual_lot_quantity_${manualLotRowCounter}"]`,
  );
  const weightInput = row.querySelector(
    `input[name="manual_lot_weight_${manualLotRowCounter}"]`,
  );
...
async function loadManualLocations() {
  try {
    ...
    const locationSelects = document.querySelectorAll(
      '#manualLotsContainer select[name^="manual_lot_location_"]',
    );
...
function updateManualTotalQuantity() {
  ...
  rows.forEach((row) => {
    const quantityInput = row.querySelector(
      'input[name^="manual_lot_quantity_"]',
    );
  ...
  document.getElementById("manualTotalWeight").textContent =
  ...
  try {
    ...
    if (!createOrderResponse.ok) {
      ...
      try {
        ...
        errorMsg =
          ...
          `ステータス ${createOrderResponse.status}`;
      }
    ...
    if (!createItemResponse.ok) {
      ...
      try {
        ...
        errorMsg =
          errorMsg?.substring(0, 200) ||
          `ステータス ${createItemResponse.status}`;
      }
    ...
    for (let i = 1; i <= manualLotRowCounter; i++) {
      ...
      const lotNumberInput = lotRow.querySelector(
        `input[name="manual_lot_number_${i}"]`,
      );
      const quantityInput = lotRow.querySelector(
        `input[name="manual_lot_quantity_${i}"]`,
      );
      const weightInput = lotRow.querySelector(
        `input[name="manual_lot_weight_${i}"]`,
      );
      const locationElement = lotRow.querySelector(
        `select[name="manual_lot_location_${i}"]`,
      );
      const notesInput = lotRow.querySelector(
        `input[name="manual_lot_notes_${i}"]`,
      );
    ...
    if (typeof showToast === "function") {
      showToast(
        `${appliedCount}件のロットを登録しました（発注番号: ${orderData.order_number}）`,
        "success",
      );
    }
...
function initializeInspectionTab() {
  document
    .getElementById("loadInspectionTargetBtn")
    ?.addEventListener("click", loadInspectionTargetByCode);
  document
    .getElementById("resetInspectionFormBtn")
    ?.addEventListener("click", resetInspectionForm);
  document
...
function createInspectionItemRow(item) {
  ...
  const totalWeight =
    initialWeight > 0 ? initialWeight : item.total_weight_kg || 0;
  const weightDisplay =
    totalWeight > 0 ? `${Number(totalWeight).toFixed(3)}kg` : "-";

  // 検品状態バッジ
  const inspectionStatus = (
    item.lot?.inspection_status || "pending"
  ).toLowerCase();
  ...
  const buttonOnClick = buttonDisabled
    ? ""
...
async function loadInspectionTargetByCode() {
  ...
  try {
    const res = await fetch(
      `/api/inspections/lots/search/${encodeURIComponent(lotNumber)}`,
    );
    ...
    if (infoEl) {
      ...
      const totalWeightCandidate =
        initialWeight != null && initialWeight > 0
          ? initialWeight
          : (item?.total_weight_kg ?? null);
      const weight =
        totalWeightCandidate != null && totalWeightCandidate > 0
          ? `${Number(totalWeightCandidate).toFixed(3)}kg`
          : "-";
      const lotNotes = item?.lot?.notes || "";
      ...
      if (inspectionStatus === "passed") {
        statusBadge =
          '<span class="px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-700">合格</span>';
      } else if (inspectionStatus === "failed") {
        statusBadge =
          '<span class="px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-700">不合格</span>';
      } else {
        statusBadge =
          '<span class="px-2 py-1 rounded-full text-xs font-medium bg-amber-100 text-amber-700">未検品</span>';
      }
    ...
    if (infoEl) {
      infoEl.innerHTML =
        '<p class="text-gray-600">ロット番号を入力して読み込んでください。</p>';
...
async function submitInspection(event) {
  ...
  // 実測値は廃止
  // 外観は廃止。曲がりはセレクトに統一
  const bendingOk = document.getElementById("bendingOkSelect")
    ? document.getElementById("bendingOkSelect").value === "true"
    : true;
  ...
  // 追加項目
  const judgementVal = document.getElementById("inspectionJudgementSelect")
    ? document.getElementById("inspectionJudgementSelect").value
    : "";
  const scratchOk = document.getElementById("scratchOkSelect")
    ? document.getElementById("scratchOkSelect").value === "true"
    : true;
  const dirtOk = document.getElementById("dirtOkSelect")
    ? document.getElementById("dirtOkSelect").value === "true"
    : true;
...
function initializePrintTab() {
  document
    .getElementById("refreshPrintBtn")
    ?.addEventListener("click", () => loadPrintableItems(1));
  document
    .getElementById("resetPrintFiltersBtn")
    ?.addEventListener("click", resetPrintFilters);

  // リアルタイム検索（入力時にdebounceで検索実行）
  document.getElementById("printMaterialFilter")?.addEventListener(
    "input",
    debounce(() => renderFilteredPrintItems(1), 300),
  );
  document.getElementById("printLotFilter")?.addEventListener(
    "input",
    debounce(() => renderFilteredPrintItems(1), 300),
  );
  ...
  document
    ...
    ?.addEventListener("click", hidePrintModal);
  document
    .getElementById("executePrintLotNumber")
    ?.addEventListener("click", executePrintLotNumber);
...
async function loadPrintableItems(page = 1) {
  ...
  try {
    ...
    const response = await fetch(
      "/api/inventory/?skip=0&limit=1000&is_active=true&has_stock=true",
    );
    if (!response.ok) {
      throw new Error(`API error: ${response.status}`);
    }
    ...
    // フィルター適用して表示
    renderFilteredPrintItems(page);
  } catch (error) {
    loading.classList.add("hidden");
    console.error("印刷可能アイテム読み込みエラー:", error);
    if (typeof showToast === "function") {
      showToast(
        "印刷可能アイテムの読み込みに失敗しました: " + error.message,
        "error",
      );
    }
  }
...
function renderFilteredPrintItems(page = 1) {
  const tableBody = document.getElementById("printItemsTableBody");
  const empty = document.getElementById("printEmpty");

  // フィルター条件取得
  const material =
    document.getElementById("printMaterialFilter")?.value?.toLowerCase() || "";
  const lot =
    document.getElementById("printLotFilter")?.value?.toLowerCase() || "";

  // フィルタリング
  const filteredItems = printAllItems.filter((item) => {
    const itemMaterial = (
      item.material?.display_name ||
      item.material?.name ||
      ""
    ).toLowerCase();
    const itemLot = (item.lot?.lot_number || "").toLowerCase();

    const matchMaterial = !material || itemMaterial.includes(material);
    const matchLot = !lot || itemLot.includes(lot);
  ...
  if (filteredItems.length === 0) {
    tableBody.innerHTML = "";
    empty.classList.remove("hidden");
    renderPagination("print", 1, 1, renderFilteredPrintItems);
    return;
  }
  ...
  // ページネーション計算
  const totalPages = Math.ceil(filteredItems.length / printPageSize);
...
async function showPrintModal(lotNumber) {
  ...
  try {
    // ロット番号からアイテム情報を取得
    const response = await fetch(
      `/api/inventory/search/${encodeURIComponent(lotNumber)}`,
    );
    if (!response.ok) {
...
function updateConversionDisplays(quantity, weight, unitWeight) {
  ...
  if (quantityEl)
    quantityEl.textContent =
      Number.isFinite(quantity) && quantity > 0 ? String(quantity) : "-";
  if (weightEl)
  ...
  if (unitWeightEl)
    unitWeightEl.textContent =
      typeof unitWeight === "number" && Number.isFinite(unitWeight)
...
function updateConversion() {
  try {
    const quantityInput = document.getElementById("quantityInput");
    const weightInput = document.getElementById("weightInput");
    const diameterInput = document.querySelector(
      'input[name="diameter_input"]',
    );
    const densityInput = document.getElementById("densityInput");
    const lengthInput = document.querySelector('input[name="length_mm"]');
    ...
    const unitWeight = roundTo3(
      calculateUnitWeightKg(
        parsedDiameter.shape,
        parsedDiameter.diameter_mm,
        lengthMm,
        density,
      ),
    );
...
function addLotRow(
  ...
  const quantityValue =
    defaultQuantity && defaultQuantity > 0 ? ` value="${defaultQuantity}"` : "";
  const weightValue =
    defaultWeight && defaultWeight > 0 ? ` value="${defaultWeight}"` : "";
...
function onLotQuantityChange(lotId) {
  const quantityInput = document.querySelector(
    `input[name="lot_quantity_${lotId}"]`,
  );
  const weightInput = document.querySelector(
    `input[name="lot_weight_${lotId}"]`,
  );
...
function updateTotalQuantity() {
  ...
  if (diameterCalcInput && shapeCalcSelect && densityInput && lengthInput) {
    ...
    if (
      ...
      try {
        unitWeight = calculateUnitWeightKg(
          shape,
          diameter_mm,
          lengthMm,
          density,
  ...
  if (unitWeight > 0) {
    ...
    if (
      (selectedMethod === "quantity" ||
        (!selectedMethod && currentOrderType === "quantity")) &&
      totalQuantity > 0
    ) {
      if (weightRefEl)
        weightRefEl.textContent = (unitWeight * totalQuantity).toFixed(3);
    }
    // 重量入力モード（または発注タイプが重量）→ 本数（参考）を表示
    if (
      (selectedMethod === "weight" ||
        (!selectedMethod && currentOrderType === "weight")) &&
      totalWeight > 0
    ) {
      const computedQty = Math.floor(totalWeight / unitWeight);
      if (qtyRefEl)
        qtyRefEl.textContent =
          Number.isFinite(computedQty) && computedQty > 0
            ? String(computedQty)
            : "-";
    }
  }
...
async function loadInspectedLotsList(page = 1) {
  ...
  try {
    const materialSpec = encodeURIComponent(
      (document.getElementById("inspectedMaterialFilter")?.value || "").trim(),
    );
    const lotNumber = encodeURIComponent(
      (document.getElementById("inspectedLotFilter")?.value || "").trim(),
    );
    const orderNumber = encodeURIComponent(
      (
        document.getElementById("inspectedOrderNumberFilter")?.value || ""
      ).trim(),
    );
    ...
    const totalPages = Math.ceil(
      inspectedLotsAll.length / inspectedLotsPageSize,
    );
    ...
    renderPagination(
      "inspectedLots",
      currentInspectedPage,
      totalPages,
      loadInspectedLotsList,
    );
  } catch (error) {
    console.error("検品済み一覧の読み込みに失敗:", error);
    loading.classList.add("hidden");
    if (typeof showToast === "function")
      showToast("検品済み一覧の読み込みに失敗しました", "error");
  }
}
...
function createInspectedLotRow(lot) {
  ...
  const weightDisplay =
    lot.total_weight_kg > 0
      ? `${(lot.total_weight_kg || 0).toFixed(3)}kg`
      : "-";
  const inspectedAt = lot.inspected_at
    ? new Date(lot.inspected_at).toLocaleString()
    : "-";
...
async function loadProcessingItems() {
  ...
  if (!tbody) return;
  tbody.innerHTML =
    '<tr><td class="px-3 py-2 text-gray-500" colspan="11">読み込み中...</td></tr>';

  const showCompleted =
    document.getElementById("showCompletedProcessing")?.checked || false;
  ...
  try {
    ...
    if (items.length === 0) {
      tbody.innerHTML =
        '<tr><td class="px-3 py-2 text-gray-500 text-center" colspan="11">データがありません</td></tr>';
      return;
    }
    ...
    for (const row of items) {
      ...
      const optionsHtml = [
        ...
        ...optionList.map(
          (opt) =>
            `<option value="${opt}" ${instructionValue === opt ? "selected" : ""}>${opt}</option>`,
        ),
      ].join("");
      ...
      saveEl.addEventListener("click", async () => {
        ...
        try {
          const resp = await fetch(
            `/api/purchase-orders/processing-items/${row.id}?instruction=${encodeURIComponent(selectEl.value)}&free_text=${encodeURIComponent(notesEl.value || "")}&worker=${encodeURIComponent(workerEl.value || "")}`,
            {
              method: "PUT",
            },
          );
      ...
      actionEl.addEventListener("click", async () => {
        if (isCompleted) {
          ...
          try {
            const resp = await fetch(
              `/api/purchase-orders/processing-items/${row.id}/complete`,
              {
                method: "POST",
              },
            );
    ...
    console.error(e);
    tbody.innerHTML =
      '<tr><td class="px-3 py-2 text-red-600" colspan="11">読み込みに失敗しました</td></tr>';
  }
}
