...
class APIClient {
  ...
  static async calculateWeight(materialId, lengthMm, quantity = 1) {
    return this.request(
      `/materials/${materialId}/calculate-weight?length_mm=${lengthMm}&quantity=${quantity}`
    );
  }
...
class FormManager {
  ...
  validateField(field) {
    ...
    if (
      ...
      value &&
      !Utils.validateInstructionNumber(value)
    ) {
      isValid = false;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // サービスワーカーの登録（PWA対応準備）- 現在は無効化
  // TODO: PWA機能を有効にする場合は適切なsw.jsファイルを作成
